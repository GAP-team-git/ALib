...
public:
    ...
    : p_shape(dims), p_dataPtr(std::make_shared<std::vector<T>>(p_shape.totalSize())) {}
    AArray<T>(const AShape& shape, T scalar)
    : p_shape(shape), p_dataPtr(std::make_shared<std::vector<T>>(p_shape.totalSize())) {}
    ...
    AArray<T> operator+(T scalar) const {
        return binaryOp(AArray<T>(shape(), scalar), [](T a, T b){return a+b;});
    }
    ...
    [[nodiscard]] virtual bool m_allEqual(const AObject& obj) const noexcept override{
        ...
        ATensorIteratorGeneric<T> it(arrays);
        bool eq = true;
        it.forEach([&eq](std::vector<T*> ptrs, size_t){
            if(*ptrs[0]!=*ptrs[1]) eq=false;
        });
        return eq;
    }
    ...
    [[nodiscard]] virtual bool m_allClose(const AObject& obj, double eps) const noexcept override{
        ...
        ATensorIteratorGeneric<T> it(arrays);
        bool close = true;
        it.forEach([eps,&close](std::vector<T*> ptrs, size_t){
            if(std::fabs(*ptrs[0]-*ptrs[1])>eps) close=false;
        });
        return close;
    }
...
protected:
    ...
    // -------------------
    // ATensorIteratorGeneric integrated
    // -------------------
    template<typename U>
...
public:
    ...
    {
        if(arrays.empty())
            throw std::invalid_argument("ATensorIteratorGeneric: no arrays provided");

        pp_shape = arrays[0]->shape();
        for(size_t i=1;i<arrays.size();++i)
            pp_shape = AShape::broadcast(pp_shape, arrays[i]->shape());

        pp_rank = pp_shape.rank();
        pp_index.assign(pp_rank,0);

        pp_strides.clear();
        for(auto* arr : pp_arrays){
            std::vector<size_t> s(pp_rank,0);
            const auto& arrDims = arr->shape().dims();
            const auto& arrStrides = arr->shape().strides();
            size_t offset = pp_rank - arrDims.size();
            for(size_t j=0;j<arrDims.size();++j){
                s[j+offset] = arrDims[j]==1 ? 0 : arrStrides[j];
            }
            pp_strides.push_back(s);
        }

        pp_data.clear();
        for(auto* arr : pp_arrays)
            pp_data.push_back(arr->raw());

        pp_offsets.assign(pp_arrays.size(),0);
        for(size_t i=0;i<pp_arrays.size();++i)
            pp_offsets[i] = pp_arrays[i]->shape().offset();
    }
        ...
        for(size_t i=0;i<total;++i){
            ...
            for(size_t a=0;a<pp_arrays.size();++a){
                ...
                for(size_t d=0;d<pp_rank;++d)
                    off += idx[d]*pp_strides[a][d];
                ptrs[a] = pp_data[a]+off;
            }
...
private:
    ...
    AShape pp_shape;
    size_t pp_rank;
    std::vector<std::vector<size_t>> pp_strides;
    std::vector<T*> pp_data;
    std::vector<size_t> pp_offsets;
