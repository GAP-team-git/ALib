...
public:
    ...
    size_t flatIndex(const std::vector<size_t>& idx) const {
        if(idx.size()!=p_dims.size()) throw std::runtime_error("AShape::flatIndex rank mismatch");
        size_t offset=0;
        for(size_t i=0;i<p_dims.size();++i){
            if(idx[i]>=p_dims[i]) throw std::out_of_range("AShape::flatIndex index out of bounds");
    ...
    bool operator==(const AShape& o) const { return p_dims==o.p_dims; }
    bool operator!=(const AShape& o) const { return !(*this==o); }
